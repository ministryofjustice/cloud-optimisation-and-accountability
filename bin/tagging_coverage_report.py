...
def build_business_unit_filter(business_unit: str) -> str:
    """Return an Athena SQL filter for a business unit, or no filter for All."""
...
def generate_query_total_tagging_coverage_for_tag(
    ...
    :param business_unit: The business unit for which to calculate coverage,
        or 'All' for an aggregated report.
    :param tag_key: The tag key to calculate coverage for.
    :return: SQL query string.
    """
    business_unit_filter = build_business_unit_filter(business_unit)
...
def generate_query_list_of_aws_accounts(
    billing_period: str,
    business_unit: str,
) -> str:
    """
...
def fetch_account_coverage(
    ...
    logger.info(
        "Processing AWS account coverage %s for tag %s",
        aws_account_name,
        tag_key,
    )
    ...
    aws_account_cov_prc_per_tag = (
        round(
            float(
                results_aws_account_per_tag["ResultSet"]["Rows"][1]["Data"][0][
                    "VarCharValue"
                ]
            ),
            2,
        ...
        if "VarCharValue"
        in results_aws_account_per_tag["ResultSet"]["Rows"][1]["Data"][0]
        else 0
    )
...
def generate_tagging_coverage_metrics(
    ...
    :return: df of total coverage per tag, and a df with coverage per-account & per-tag.
    """
    ...
    total_tag_coverage = {}
    df_tagging_coverage_aws_accounts = df_aws_accounts.rename(
        columns={
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for tag_key, coverage in total_tag_coverage.items():
            if coverage > 0:
                ...
                logger.info(
                    "Completed per-account tagging coverage for tag %s",
                    tag_key,
                )
    ...
    df_tagging_coverage_aws_accounts = df_tagging_coverage_aws_accounts.sort_values(
        by=["business_unit", "AWS_account_name"]
...
def generate_excel_report(
    ...
    :param df_total_tagging_coverage: DataFrame with overall coverage per tag.
    :param df_tagging_coverage_aws_accounts: DataFrame with per-account coverage.
    :param business_unit: Business unit label used in the output filename.
    ...
    with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
        df_total_tagging_coverage.to_excel(
            writer,
            sheet_name="Total Coverage per Tag",
            index=False,
        )

        df_tagging_coverage_aws_accounts.to_excel(
            writer,
            sheet_name="Account Coverage per Tag",
            index=False,
        )
        ...
        chart.add_series(
            {
                "name": "Coverage (%)",
                "categories": [
                    "Total Coverage per Tag",
                    1,
                    0,
                    len(df_total_tagging_coverage),
                    0,
                ],
                "values": [
                    "Total Coverage per Tag",
...
def generate_tagging_coverage_report(
    ...
    logger.info(
        "Tagging coverage report generated for business unit: %s",
        business_unit,
    )
    ...
    SlackService(os.getenv("ADMIN_SLACK_TOKEN")).send_report_with_message(
        ...
        message=(
            f"Tagging coverage report for {business_unit}\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--business_unit",
        required=True,
        help="Business unit for the report.",
    )
    parser.add_argument(
        "--billing_period",
        required=True,
        help="Billing period for the report.",
    )
