...
class SlackService:
    ...
    # Added to stop TypeError on instantiation.
    # See:
    # https://github.com/python/cpython/blob/d2340ef25721b6a72d45d4508c672c4be38c67d3/Objects/typeobject.c#L4444
    ...
    def send_nonprod_resource_wastage_alerts(self, db_wastage_ns, pod_wastage_ns):
        ...
        db_section = (
            ...
            f"outside of work hours in the following namespaces:*\n"
            f"{db_wastage_ns}"
        )
        pod_section = (
            f"📌*{len(pod_wastage_ns)} POD instances without scheduled downtime "
            f"when the database is turned off at night in the following namespaces:*\n"
            f"{pod_wastage_ns}"
        )
